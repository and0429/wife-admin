...
<script>
import { listItem, getItem, delItem, addItem, updateItem, exportItem } from '@/api/system/item'
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除标准类目编号为"' + ids + '"的数据项?', '警告', {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有标准类目数据项?', '警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(function() {
